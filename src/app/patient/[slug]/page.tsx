...
const handleConfirmedSubmit = useCallback(async () => {
  ...
  const handleLoadingData = useCallback(async () => {
    ...
    if (!isNaN(id)) {
      ...
      const changedNotes = {
        ...patientData?.notes,
        allergy: patientData?.notes?.allergy ? patientData?.notes?.allergy.map((item: any) => item.replace(/'/g, ''))
          .join(', ') : ''
      };
      setNotes(changedNotes);
    }
  }, [token, slug, getPatientById]);

  useEffect(() => {
    if (token && slug && slug !== "new") {
      handleLoadingData();
    }
          ...
          <div className="w-1/2">
            ...
            <div className="flex items-center">
              <div className="flex items-center gap-2">
                {patient?.status !== undefined && patient?.status !== null && (
                  <>
                    <ButtonSelector
                      change={(checked) => {
                        if (!isEditing) return;
            ...
            <div className="flex gap-12 mb-5">
              <div className="w-1/2">
                <label
                  htmlFor="tipoSanguinio"
                  className="block mb-1 text-xl font-medium required"
                >
                  <span className="text-red-500">*</span>
                  <span className="text-black"> Tipo Sanguíneo:</span>
                </label>
                <input
                  type="text"
                  className={`w-full px-6 py-4 rounded-lg text-lg focus:outline-none ${!isEditing ? 'bg-gray-300 text-gray-500 cursor-not-allowed' : ''}`}
                  ...
                  value={notes?.bloodType ?? ""}
                  disabled={!isEditing}
                  required
                  onChange={(event) =>
                    setNotes((oldValue: any) => ({
                      ...oldValue,
            ...
            <div className="w-full mt-10 px-4 md:px-0">
              ...
              >
                <span className="text-red-500">*</span>
                <span className="text-black"> Alergias:</span>
              </label>
              <div className="max-w-3xl mx-auto">
                <textarea
                  className={`w-full h-[200px] p-6 rounded-lg text-lg focus:outline-none resize-none
