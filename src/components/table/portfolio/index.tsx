...
interface TableProps {
  ...
  onPayOrReceive: (id: number, newStatus: string) => Promise<void>; // <- função para atualizar o status no banco
}

export function TableExpenses({ data, onDelete, onPayOrReceive }: TableProps) {
  ...
  const [actionType, setActionType] = useState<"delete" | "payOrReceive" | null>(null);
  ...
  const handleDeleteClick = (id: number) => {
    ...
    setConfirmationMessage("Você deseja realmente excluir esse título?");
    setIsConfirmOpen(true);
  };

  const handleConfirmDelete = async () => {
    if (selectedId !== null && actionType === "delete") {
  ...
  const handlePayOrReceiveClick = (id: number, type: string) => {
    const item = data.find((x) => x.id === id);

    if (!item) return;

    if (item.status === "PAGO" && type === "Conta a Pagar") {
      setAlertMessage("Esse título já foi pago!");
      setAlertOpen(true);
      return;
    }

    if (item.status === "RECEBIDO" && type === "Conta a Receber") {
      setAlertMessage("Esse título já foi recebido!");
      setAlertOpen(true);
      return;
    }
    ...
    setActionType("payOrReceive");

    if (type === "Conta a Pagar") {
      setConfirmationMessage("Você deseja realmente pagar esse título?");
    } else if (type === "Conta a Receber") {
      setConfirmationMessage("Você deseja realmente receber esse título?");
    } else {
      setConfirmationMessage("Você deseja realmente realizar esta ação?");
    }

    setIsConfirmOpen(true);
  };

  const handleConfirmPayOrReceive = async () => {
    if (selectedId !== null && actionType === "payOrReceive" && selectedType) {
      const newStatus = selectedType === "Conta a Pagar" ? "PAGO" : "RECEBIDO";

      await onPayOrReceive(selectedId, newStatus);
      setIsSuccessOpen(true);
    }
    closeConfirmation();
  };
  ...
  const handleConfirm = () => {
    if (actionType === "delete") {
      handleConfirmDelete();
    } else if (actionType === "payOrReceive") {
      handleConfirmPayOrReceive();
    }
  };
            ...
            {paginatedData.length === 0 ? (
              ...
              paginatedData.map((item: any, i: number) => (
                <tr className={`${i % 2 === 0 ? "bg-white" : "bg-gray-100"}`} key={item?.id}>
                  ...
                  <td className="text-center py-4">
                    {item?.releaseDate
                      ? DateTime.fromISO(item.releaseDate).setLocale('pt-BR').toFormat("cccc, dd 'de' LLLL 'de' yyyy")
                      : '-'}
                  </td>
                  ...
                  <td className="text-center py-4">
                    <div className={`rounded-2xl px-4 py-2 text-white flex justify-center items-center gap-2
                        ${item?.status === "PENDENTE" ? "bg-red-400" :
                        item?.status === "PAGO" ? "bg-green-500" :
                          item?.status === "RECEBIDO" ? "bg-blue-500" :
                            "bg-gray-500" // cor padrão para status desconhecido
                      }
                      `}>
                      <div className={`
                          ${item?.status === "PENDENTE" ? "bg-red-800" :
                          item?.status === "PAGO" ? "bg-green-800" :
                            item?.status === "RECEBIDO" ? "bg-blue-800" :
                              "bg-gray-800"
                        } 
                  ...
                  <td className="text-center w-[5%] px-4" onClick={(e) => e.stopPropagation()}>
                    <div className="flex gap-3">
                      ...
                      <div
                        ...
                        onClick={() => handlePayOrReceiveClick(item.id, item.type)}
                      >
